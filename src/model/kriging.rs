...
mod tests {
    ...
    #[test]
    fn ordinary_kriging_preserves_a_constant_field() {
        let samples = [
            KrigingSample {
        ...
        let progress = crate::model::progress::Progress::default();
        let phase = progress.phase(0.0, 1.0);
        let result = ordinary_kriging(
            &samples,
            KrigingGrid {
                lower: DVec3::ZERO,
                upper: DVec3::new(10.0, 10.0, 10.0),
                cell: DVec3::splat(10.0),
            },
        ...
        .unwrap();
        assert!((result.estimates[0] - 3.5).abs() < 1.0e-9);
    }

    #[test]
    fn grid_dimensions_round_up_to_cover_bounds() {
        let grid = KrigingGrid {
            lower: DVec3::ZERO,
            upper: DVec3::new(21.0, 20.0, 9.0),
            cell: DVec3::splat(10.0),
        };
        assert_eq!(grid.dimensions().unwrap(), [3, 2, 1]);
    }
}
