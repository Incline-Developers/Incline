...
pub(crate) fn draw_new_reserve_field_dialog(ui: &mut egui::Ui, editor: &mut EditorState, document: &Document, commands: &mut Vec<UiCommand>) {
    ...
    let mut open = true;
    let mut close = false;
    DragableMenu::new("new_reserve_field_dialog", tr!(literal = "New Field")).open(&mut open).min_width(300.0).show(ui.ctx(), |ui| {
        ...
        .show(ui);
        let sum_fields: Vec<_> = document.reserve_fields().iter().filter(|field| matches!(field.aggregation, ReserveAggregation::Sum)).collect();
        if editor.new_reserve_field_kind == ReserveFieldKind::WeightedAverage {
            if editor
                .new_reserve_field_weight_field
                .is_none_or(|id| !sum_fields.iter().any(|field| field.id == id))
            {
        ...
        menu::menu_actions(ui, |ui| {
            let can_add =
                !editor.new_reserve_field_name.trim().is_empty() && (editor.new_reserve_field_kind != ReserveFieldKind::WeightedAverage || editor.new_reserve_field_weight_field.is_some());
